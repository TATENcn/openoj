...
新增 `sqlx = 0.9.0`，关闭 default features，生产依赖仅启用 `postgres`、`runtime-tokio`、`migrate` 和 `tls-rustls-ring-native-roots`；migration 通过 `include_str!`、`Migrator::with_migrations` 和运行时 checksum 嵌入二进制，不使用在线 query macro。测试依赖额外启用 `macros`，只用于 SQLx 隔离数据库 harness。它要求 Rust 1.94，低于 workspace 1.97，许可证为 MIT OR Apache-2.0。新增 `tokio = 1.53.1`，关闭 default features，library/test/CLI 按需启用 `rt-multi-thread`、`macros`、`fs`，许可证为 MIT。两者来自 crates.io 官方仓库并由 `Cargo.lock` 固定。
